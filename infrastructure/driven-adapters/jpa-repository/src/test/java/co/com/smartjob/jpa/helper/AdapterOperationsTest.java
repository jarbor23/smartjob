...
import co.com.smartjob.jpa.JPARepository;
import co.com.smartjob.jpa.JPARepositoryAdapter;
...
class AdapterOperationsTest {
    ...
    void testSave() {
        ...
        EntityUser objectValue = EntityUser.builder().email("test").name("test").build();
        User response = User.builder().build();
        when(repository.save(objectValue)).thenReturn(objectValue);

        Object result = adapter.save(response);
